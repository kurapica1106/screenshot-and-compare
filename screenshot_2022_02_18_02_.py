...
class window_full_screen(tkinter.Tk):
    ...
    def get_screenshot(self):
        ...
        self.image_binary = sqlite3.Binary(self.image_corp_screen)
        self.image_pattern = (1, self.x1, self.y1, self.x2, self.y2,
                              self.image_binary)
        ...
        '''
        if c.execute()
        c.execute("insert into image (index, x1, y1, x2, y2, )")
        with open("compare_pattern", "w+") as file:
            self.image_file = file.read()
            self.image_file = list(self.image_file)
            self.image_file.append(self.image_pattern)
            file.write(self.image_file)
        '''
        self.window_main.deiconify()
        self.destroy()
